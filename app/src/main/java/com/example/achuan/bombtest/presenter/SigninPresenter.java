...
public class SigninPresenter extends RxPresenter<SigninContract.View>
    ...
    public void getCourseData() {
        BmobHelper.getInstance().courseBmobQueryAll().findObjects(new FindListener<CourseBean>() {
            @Override
    ...
    public void getSearchCourseData(String keyword) {
        BmobHelper.getInstance().courseBmobQueryFromKeyword(keyword).findObjects(new FindListener<CourseBean>() {
            @Override
            public void done(List<CourseBean> list, BmobException e) {
                if(e==null){
                    mView.showContent(list);
